...
public class KnowledgeIngestService {
    ...
    public KnowledgeIngestService(
            ...
            @Value("${knowledge.summary-model:qwen3.5:0.8b}") String summaryModelName,
    ...
    private RuntimeException mapIngestRuntimeException(RuntimeException e) {
        ...
        if (lower.contains("ollama") || lower.contains("embedding")) {
            String detail = extractOllamaDetail(msg);
            if (lower.contains("failed to resolve") || lower.contains("unknownhost") || lower.contains("unknown host") || lower.contains("name or service not known") || lower.contains("connection refused")) {
                return new IllegalArgumentException("向量化失败：无法连接本地 Ollama 服务（localhost:11434）。请确认 Ollama 服务已启动。" + detail);
            }
            if (lower.contains("http 400")) {
                return new IllegalArgumentException("向量化失败：Ollama Embeddings 请求参数错误（HTTP 400）。" + detail);
            }
            if (lower.contains("model") && (lower.contains("not found") || lower.contains("does not exist"))) {
                return new IllegalArgumentException("向量化失败：Embedding 模型不存在。请确认 Ollama 已拉取 embedding 模型（如 bge-m3）。" + detail);
            }
            return new IllegalArgumentException("向量化失败：Ollama Embeddings 调用失败。" + detail);
        }
    ...
    private String extractOllamaDetail(String msg) {
    ...
    private String generateSummaryWithFallback(List<Message> messages, String rawModel) {
        ...
        if (rawModel == null || rawModel.isBlank() || ModelPolicy.isSmall(rawModel)) {
            candidates = ModelPolicy.autoNonVisionOnceCandidates();
        } else if (ModelPolicy.isMedium(rawModel)) {
            candidates = List.of(new ModelPolicy.ModelCandidate("qwen3.5:2b", ModelPolicy.Provider.OLLAMA, false, true, 5));
        } else if (ModelPolicy.isLarge(rawModel)) {
            candidates = List.of(new ModelPolicy.ModelCandidate("qwen3.5:4b", ModelPolicy.Provider.OLLAMA, false, true, 1));
        } else {
        ...
        for (int i = 0; i < candidates.size(); i++) {
            ...
            try {
                String s = ollamaService.chatOnce(messages, c.modelName()).block();
                if (s != null && !s.isBlank()) return s;
            } catch (Exception ignored) {
            }

            boolean canFallback = i + 1 < candidates.size();
            if (!canFallback) {
                break;
            }
        }
